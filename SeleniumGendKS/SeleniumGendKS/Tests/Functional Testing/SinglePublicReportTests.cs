...
{
    [TestFixture]
    internal class SinglePublicReportTests : BaseFunctionTest
    {
        ...
        public void ST001_SinglePublicReport()
        {
            ...
            var report = WorkbenchApi.SinglePublicReport(body, msalIdtoken);
            Assert.That(report.StatusCode, Is.EqualTo(HttpStatusCode.OK));

            // Parse IRestResponse to JObject
            JObject reportJs = JObject.Parse(report.Content);
            ClassicAssert.AreEqual("1999", reportJs["Yearly_Return_Metrics"][0]["Year"].ToString());
            Assert.That(reportJs["Yearly_Return_Metrics"].Count(), Is.EqualTo(25));
            Assert.That(reportJs["Return_by_Duration"].Count(), Is.EqualTo(40));
            Assert.That(reportJs["Autoscore"].Count(), Is.EqualTo(3));
            Assert.That(reportJs["Annual_Return_Section"].Count(), Is.EqualTo(5));
            Assert.That(reportJs["Annual_Risk_Section"].Count(), Is.EqualTo(5));
            Assert.That(reportJs["Drawdown_Statistics"].Count(), Is.EqualTo(5));
            Assert.That(reportJs["Rolling_Correlation"].Count(), Is.EqualTo(4));
            Assert.That(reportJs["Annualized_Correlation"].Count(), Is.EqualTo(4));
            Assert.That(reportJs["Drawdown_Metrics"].Count(), Is.EqualTo(7));
            Assert.That(reportJs["Drawdown_Graph"].Count(), Is.EqualTo(574));
            Assert.That(reportJs["Correlation_Graph"].Count(), Is.EqualTo(1104));
            Assert.That(reportJs["Monthly_Return_Histograms"].Count(), Is.EqualTo(287));
            Assert.That(reportJs["Fund_Aums"].Count(), Is.EqualTo(48));
            Assert.That(reportJs["Raw_Data"].Count(), Is.EqualTo(287));
            Assert.That(reportJs["fund_info"].Count(), Is.EqualTo(8));
            //JObject reportJsBL = JObject.Parse(File.ReadAllText(filePath + fileName));
            //reportJsBL["fund_info"]["Fund AUM"] = reportJs["fund_info"]["Fund AUM"];
            //var sortReportJs = new JObject(reportJs.Properties().OrderBy(p => (string?)p.Name));
        ...
        [Test, Category("API Smoke Tests")]
        public void ST002_SinglePublicReport_Solovis()
        {
            #region Variables declare
            const string fundId = "565"; //Laurion
            ...
            // Parse IRestResponse to JObject
            JObject reportJs = JObject.Parse(report.Content);
            ClassicAssert.AreEqual("2005", reportJs["Yearly_Return_Metrics"][0]["Year"].ToString());
            Assert.That(reportJs["Yearly_Return_Metrics"].Count(), Is.EqualTo(18));
            Assert.That(reportJs["Return_by_Duration"].Count(), Is.EqualTo(40));
            Assert.That(reportJs["Autoscore"].Count(), Is.EqualTo(3));
            Assert.That(reportJs["Annual_Return_Section"].Count(), Is.EqualTo(4));
            Assert.That(reportJs["Annual_Risk_Section"].Count(), Is.EqualTo(4));
            Assert.That(reportJs["Drawdown_Statistics"].Count(), Is.EqualTo(4));
            Assert.That(reportJs["Rolling_Correlation"].Count(), Is.EqualTo(3));
            Assert.That(reportJs["Annualized_Correlation"].Count(), Is.EqualTo(3));
            Assert.That(reportJs["Drawdown_Metrics"].Count(), Is.EqualTo(6));
            Assert.That(reportJs["Drawdown_Graph"].Count(), Is.EqualTo(416));
            Assert.That(reportJs["Correlation_Graph"].Count(), Is.EqualTo(591));
            Assert.That(reportJs["Monthly_Return_Histograms"].Count(), Is.EqualTo(208));
            Assert.That(reportJs["Fund_Aums"].Count(), Is.EqualTo(25));
            Assert.That(reportJs["Raw_Data"].Count(), Is.EqualTo(208));
            Assert.That(reportJs["fund_info"].Count(), Is.EqualTo(7));
            //JObject reportJsBL = JObject.Parse(File.ReadAllText(filePath + fileName));
            //reportJsBL["fund_info"]["Fund NAV"] = reportJs["fund_info"]["Fund NAV"];
            //var sortReportJs = new JObject(reportJs.Properties().OrderBy(p => (string?)p.Name));
            //var sortReportJsBL = new JObject(reportJsBL.Properties().OrderBy(p => (string?)p.Name));
            //Assert.AreEqual(sortReportJs, sortReportJsBL);
            #endregion
        ...
        [Test, Category("API Smoke Tests")]
        public void ST003_SinglePublicReport_AlternativesEvestment()
        {
            #region Variables declare
            ...
            // Parse IRestResponse to JObject
            JObject reportJs = JObject.Parse(report.Content);
            ClassicAssert.AreEqual("2016", reportJs["Yearly_Return_Metrics"][0]["Year"].ToString());
            #endregion
        }

        [Test, Category("API Smoke Tests")]
        public void ST004_SinglePublicReport_TraditionalEvestment()
        {
            #region Variables declare
            ...
            // Parse IRestResponse to JObject
            JObject reportJs = JObject.Parse(report.Content);
            ClassicAssert.AreEqual("2006", reportJs["Yearly_Return_Metrics"][0]["Year"].ToString());
            #endregion
        }
        #endregion
    }
